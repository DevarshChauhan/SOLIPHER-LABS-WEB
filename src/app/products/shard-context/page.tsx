...
const currentGaps = [
  ...
  "p95 compile latency is now measured (see Real today), and it surfaced a real, unfixed problem: at the architecture's own maximum caps (12 mandatory atoms, 32 candidates), compile() runs roughly 2-3x over its own stated combined stage budget. A plausible cause was identified by reading the solver's code, not yet profiled or fixed, since the affected component is security- and determinism-critical and deserves its own dedicated pass. No dedicated controller microbenchmark (Stratum A) exists yet either. The ablations baseline is real for three of its four components; the fourth, stable-prefix policy, isn't exercised since no real caching mechanism exists in this codebase yet to disable.",
];
            ...
            <div className="rounded-2xl border border-border bg-background p-6">
              ...
              <ul className="mt-3 space-y-2.5">
                {[
                  ...
                  "Security CI gates wired into real automation: fmt, clippy, forbid(unsafe_code), overflow checks, 503 tests, fuzzing, dependency scanning",
                  ...
                  "Real p95 compile-latency measurement, 5,000 iterations per scenario, real release-build numbers, reported even though the result at the architecture's own maximum caps wasn't flattering: roughly 2-3x over its own stated budget, with a plausible cause (not yet fixed) named honestly",
